...
class FileUtils:
    ...
    async def download_file_from_url(self, file_url: str) -> str:
        ...
        async with AsyncHttpClient(follow_redirects=True) as client:
            try:
                ...
                file_name = self._generate_safe_file_name(original_name)
    ...
    async def save_uploaded_file(self, file: Union[UploadFile, bytes], file_name: str) -> str:
        ...
        if isinstance(file, (bytes, bytearray)):
            return await self.save_file(file, file_name)
        ...
        try:
            async with aiofiles.open(file_path, 'wb') as output_file:
                while True:
                    ...
                    if self.LIMIT_FILE_SIZE and total_size > self.MAX_FILE_SIZE:
                        error_msg = f"File size exceeds the limit: {total_size} > {self.MAX_FILE_SIZE}"
                        self.logger.error(error_msg)
                        raise ValueError(error_msg)
    ...
    async def get_audio_duration(self, temp_file_path: str) -> float:
        ...
        try:
            ...
            duration = await self._get_duration_with_ffprobe(temp_file_path)
            self.logger.debug(f"Audio file duration: {duration:.2f} seconds")
            return duration
        except FileNotFoundError:
            self.logger.warning(
                "ffprobe binary '%s' was not found. Falling back to the WAV parser.",
                self.FFPROBE_BINARY,
            )
        except ValueError as e:
            self.logger.warning(
                "ffprobe could not determine the duration for %s: %s. "
                "Trying the WAV fallback.",
                temp_file_path,
                str(e),
            )

        try:
            duration = await asyncio.to_thread(self._get_duration_from_wav, temp_file_path)
            self.logger.debug(f"Audio file duration: {duration:.2f} seconds")
            return duration
        except Exception as e:
    ...
    async def _get_duration_with_ffprobe(self, file_path: str) -> float:
        ...
        if process.returncode != 0:
            error_message = stderr.decode().strip() or "ffprobe failed to inspect the file."
        ...
        try:
            ...
            raise ValueError(f"Invalid ffprobe duration output: {output}") from exc

        if duration < 0:
            raise ValueError("ffprobe returned a negative duration.")

        return duration

    def _get_duration_from_wav(self, file_path: str) -> float:
        try:
            ...
            raise ValueError(
                "Fallback duration probing only supports valid WAV files when ffprobe "
                "is unavailable or cannot parse the input."
            ) from exc
